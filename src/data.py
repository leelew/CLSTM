...
def make_forcing_colm_fluxnet(
    ...
    # must use HH dataset to get RH for Q
    df = pd.read_csv(PATH, usecols=forcing_params)

    df[df == -9999.000] = np.nan
    # remove the first unstable year, may raise error in some short length site
    df = df[17568:]

    df = df.interpolate(method='linear')

    # read and make forcing
    solar = np.array(df['SW_IN_F_MDS'])[:, np.newaxis]
    frl = np.array(df['LW_IN_F_MDS'])[:, np.newaxis]
    prcp = np.array(df['P_F']/(3600*24))[:, np.newaxis]  # mm/day - mm/s
    tm = np.array(df['TA_F_MDS']+273.15)[:, np.newaxis]  # deg C - kelvin
    us = np.array(df['WS_F'])[:, np.newaxis]
    vs = np.array(np.zeros((len(solar), 1)))
    pres = np.array(df['PA_F'])[:, np.newaxis]*1000  # kpa - pa
    qm = np.array(df['RH'])[:, np.newaxis]

    E = np.exp(17.67*(tm-273.15)/((tm-273.15)+243.4))*6.112
    e = qm/100*E
    q = (0.622*e)/(pres/1000-0.378*e)

    # make daily forcing TODO: provide flag for HH and DD.
    forcing_colm = np.concatenate(
        [solar, frl, prcp, tm, us, vs, pres, q], axis=-1)
    assert np.isnan(forcing_colm).any() == True

    forcing_colm_daily = np.full(
        (int((forcing_colm.shape[0]+1)/48), forcing_colm.shape[1]), np.nan)

    for i in range(int((forcing_colm.shape[0]+1)/48)):
        forcing_colm_daily[i, :] = np.mean(forcing_colm[i*48:i*48+48], axis=0)

    solar = forcing_colm_daily[:, 0]
    frl = forcing_colm_daily[:, 1]
    prcp = forcing_colm_daily[:, 2]
    tm = forcing_colm_daily[:, 3]
    us = forcing_colm_daily[:, 4]
    vs = forcing_colm_daily[:, 5]
    pres = forcing_colm_daily[:, 6]
    q = forcing_colm_daily[:, 7]

    # fmt can't split with space!
    np.savetxt(site_name+'.txt',
               np.column_stack(
                   (solar, frl, prcp, tm, us, vs, pres, q)),
               fmt='%7.1f%7.1f%14.3e%10.3f%10.3f%10.3f%10.1f%12.3e')
"""
